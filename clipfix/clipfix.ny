(setq input-file (read-line))
(setq output-file (read-line))
(setf thresh 95)

;; -10dB for headroom
(setq s (scale-db -10 (s-read input-file)))
...
;; End of Audacity plugin



;; Re-amplify for new peak
(setf amplify (- (linear-to-db
    (min (peak (aref s 0) largenumber)
         (peak (aref s 1) largenumber))
)))

(s-save (scale-db amplify s) output-file)
(exit)
